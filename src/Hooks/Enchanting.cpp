...
namespace
{
	void SetMostExpensiveEffect(RE::BSTArray<RE::Effect*>& effects, float costliest, RE::BSTArray<RE::Effect*>& a_result)
	{
			...
			case RE::ActorValue::kDestruction:
			{
				...
				case RE::ActorValue::kNone:
					if (effect->cost > oldCostliestDestruction) {
						...
						largestCost += effect->cost;
						effect->cost = largestCost;
						cheapest = effect;
		...
		for (auto* effect : effects) {
			if (effect->baseEffect != cheapest->baseEffect) {
				effect->cost = 0.0f;
			}
			a_result.push_back(effect);
		}
	}

	void CreateNewEffectArray(
		RE::BSTArray<RE::Effect>& effects,
		float costliest,
		RE::BSTArray<RE::Effect*>& a_result)
	{
		RE::ActorValue lastSchool = RE::ActorValue::kNone;
		RE::Effect* cheapest = nullptr;
		...
		RE::CraftingSubMenus::EnchantConstructMenu* a_menu)
	{
		...
		case RE::FormType::Weapon:
			...
			if (isStaff) {
				...
				_loggerInfo("Costliest: {}", _debugEDID(costliest->baseEffect));
